...
class LineConv1D(object):
    ...
    def __init__(self, window_width, window_height, window_margin,
        ...
        self.smooth_factor = smooth_factor          # moving average over last recent centers
    ...
    def search_window_space(self, signal, line_center, cols):
        ...
        max_idx = int(min(line_center + center + self.margin, cols))
    ...
    def find_window_centroids(self, warped_img):
        ...
        # moving average of last n frames(smooth_factor): avoids estimated poisition from being noisy and fluctuating
    ...
    def draw_centroids(self, warped_img, window_centroids):
        ...
        zero_ch  = np.zeros_like(template)
        ch_data  = cv2.merge((zero_ch,template,zero_ch))                # window pixels green via template in channel of RGB format
        template = np.array(ch_data,np.uint8)
        ch_data  = cv2.merge((warped_img,warped_img,warped_img))        # making original road pixels 3 color channels
        warpage  = np.array(ch_data,np.uint8)
        result   = cv2.addWeighted(warpage, 0.8, template, 0.5, 0.0)      # overlay orignal road image with window results
        return result


class LineCurvature(object):
    """
    Determine Lane Boundaries and Cuvature from given windows centroids via Conv 1D Method
    """
    def __init__(self, window_width, window_height, window_margin,
    ...
    def fit_lanes(self, left_fitx, right_fitx):
        ...
        set_lanes_width = lambda lx_line, rx_line, y_pix, window_width: np.array(
            list( zip( np.concatenate((lx_line+self.window_width/2, rx_line[::-1]-self.window_width/2), axis=0),
                       np.concatenate((y_pix, y_pix[::-1]), axis=0) )) , np.int32 )

        left_lane  = set_lanes(left_fitx,  self.yvals, self.window_width)
        right_lane = set_lanes(right_fitx, self.yvals, self.window_width)
        inner_lane = set_lanes_width(left_fitx, right_fitx, self.yvals, self.window_width)
    ...
    def fit_poly(self, x_line, yvals, res_yvals):
        ...
        A,B,C = (line_fit[0], line_fit[1], line_fit[2])
        line_fitx = A*(yvals**2) + B*yvals + C
    ...
    def annotate_frame(self, img, window_centroids, left_fitx, right_fitx):
        ...
        avg_radius_curv    = (left_radius_curv + right_radius_curv)/2
        ...
        fmt_left_curvature   = 'Left Lane Radius of Curvature = {}m'.format(round(left_radius_curv,3))
        fmt_right_curvature  = 'Right Lane Radius of Curvature = {}m'.format(round(right_radius_curv,3))
        fmt_avg_curvature    = 'Avg Lane Radius of Curvature = {}m'.format(round(avg_radius_curv,3))
        fmt_offset = 'Vehicle is: {}m {} of center'.format(abs(round(center_diff,3)), lane)
        fmt = [fmt_left_curvature, fmt_right_curvature, fmt_avg_curvature, fmt_offset]
        [place_text(img, fmt_it, (50,it)) for fmt_it, it in zip(fmt, range(50,250,50)) ]
        return img
